...
class modK2FilterHelper {
	...
	function getTitles(&$params, $restcata = 0) {
	...
	function treeselectbox_multi_select(&$params, $id = 0, $level = 0, $i, $moduleId) {
		...
		if ($moduleParams->get("restrict") == 1) {
			if ($moduleParams->get("restmode") == 0 && $moduleParams->get("restcat") != '') {
				...
				if($restsub == 1) {
					...
					$myquery .= " AND i.catid = " . $category_id;
				}
			}
		}
	...
	function getK2StorePriceValues($moduleParams) {
