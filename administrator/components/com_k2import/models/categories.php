...
class K2ModelCategories extends JModelLegacy
{
    ...
    function getData()
    {
        ...
        if ($search)
        {
        	//Artur Neumann www.individual-it.net info@individual-it.net  18.08.2013
        	//changed from 	K2_JVERSION to the joomla standard function
            $escaped = version_compare( JVERSION, '1.6', 'le' ) ? $db->escape($search, true) : $db->escape($search, true);
    ...
    function getTotal()
    {
        ...
        if ($search)
        {
            $escaped = K2_JVERSION == '15' ? $db->escape($search, true) : $db->escape($search, true);
