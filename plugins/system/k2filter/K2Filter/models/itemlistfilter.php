...
class K2ModelItemlistfilter extends K2Model {
		...
		function getData($ordering = NULL) {
							...
							$i = $matches[1];
							   ...
							   $search = JRequest::getVar('array'.$i, null);
                                ...
                                if (!empty($search_az)) {
									...
									$sql  = " AND (i.extra_fields REGEXP '^.*\"{$i}\",\"value\":\"(".implode("|", $search_az).").*\".*$')";
									$query .= $sql;
								}
								...
								if(!empty($title)) {
									...
									foreach($titles as $k => $title) {
										$query .= " AND UPPER(i.title) REGEXP UPPER('^.*{$title}.*$')";
									}
								}
								...
								if(!empty($ftext)) {
									$query .= " AND (UPPER(i.introtext) REGEXP UPPER('^.*{$ftext}.*$') OR UPPER(i.fulltext) REGEXP UPPER('^.*{$ftext}.*$'))";
								}								
								...
								///item all in one
								$fitem_all = addslashes(JRequest::getString('fitem_all'));
								$fitem_all = str_replace("+", "\\\\+", $fitem_all);
								...
								if(!empty($fitem_all)) {
									...
									$fitem_all_upper = mb_strtoupper($fitem_all, "utf-8");
									$fitem_all_lower = mb_strtolower($fitem_all, "utf-8");													
									$fitem_all_upper_first = mb_substr($fitem_all_upper, 0, 1, "utf-8") . mb_substr($fitem_all_lower, 1, strlen($fitem_all), "utf-8");
									...
									//title
										if (JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_falang'.DS.'falang.php')) {
											...
											$sql = " SELECT reference_id FROM #__falang_content as fc LEFT JOIN #__languages as fl ON fc.language_id = fl.lang_id";
											$sql .= " WHERE fc.value REGEXP '^.*{$fitem_all}.*$'";
											...
											$query .= " OR (i.title REGEXP '^.*{$fitem_all}|{$fitem_all_upper}|{$fitem_all_lower}|{$fitem_all_upper_first}.*$')";
										}
									//text
										$query .= " OR (i.introtext REGEXP '^.*{$fitem_all}|{$fitem_all_upper}|{$fitem_all_lower}|{$fitem_all_upper_first}.*$')";
										$query .= " OR (i.fulltext REGEXP '^.*{$fitem_all}|{$fitem_all_upper}|{$fitem_all_lower}|{$fitem_all_upper_first}.*$')";
									//extrafields
										$query .= " OR (";
										$words = explode(" ", $fitem_all);
										foreach($words as $k=>$word) {
											$word_upper = mb_strtoupper($word, "utf-8");
											$word_lower = mb_strtolower($word, "utf-8");													
											$word_upper_first = mb_substr($word_upper, 0, 1, "utf-8") . mb_substr($word_lower, 1, strlen($word), "utf-8");											
											if($k != 0) {
												$query .= " AND ";
											}
											$query .= "i.extra_fields_search REGEXP '^.*({$word}|{$word_upper}|{$word_lower}|{$word_upper_first}).*$'";
										}
										$query .= ")";
									//author
										$authors_list = K2ModelItemlistfilter::getAuthors($moduleParams);
										foreach($authors_list as $author) {
											if ((strpos(mb_strtolower($author->name), mb_strtolower($fitem_all)) !== false) || (strpos(mb_strtolower($author->username), mb_strtolower($fitem_all)) !== false)) {
								...
								if($rating_from) {
									...
									if (JPluginHelper::isEnabled('system', 'k2multirate')) {
										$query .= " AND (SELECT SUM(rating_sum) FROM #__k2_multirating WHERE itemid = i.id AND ratename != 'updown')
													 /(SELECT COUNT(ratename) FROM #__k2_multirating WHERE itemid = i.id AND ratename != 'updown')
													 >= {$rating_from}
												  ";
									}
									else {
										$query .= " AND (r.rating_sum/r.rating_count) >= {$rating_from}";	
									}
								}
								...
								if($rating_to) {
									...
									if (JPluginHelper::isEnabled('system', 'k2multirate')) {
										...
										$query .= " AND ((r.rating_sum/r.rating_count) <= {$rating_to} OR r.rating_count is null)";	
									}
								}
			...
			// ADDED k2FSM from here ->>
				$order = JRequest::getVar("orderby", $moduleParams->ordering_default);				
				$order_method = JRequest::getVar("orderto", $moduleParams->ordering_default_method);
		...
		function getTotal($sub_cat = 0) {
				...
				if($sub_cat != 0) {
					$cat_tree = K2ModelItemListFilter::getCategoryTree($sub_cat);
					$query .= " AND (i.catid IN (" . implode(", ", $cat_tree) . ")";
					//added for additional categories plugin
					if (JPluginHelper::isEnabled('k2', 'k2additonalcategories')) {
						$query .= " OR ca.catid IN (" . implode(", ", $cat_tree) . ")";
					}
					$query .=" )";
				}
				// <<- ADDED K2FSM till here
	...
	function prepareFilterArray($searches2, $badchars, $slider, $array, $count, $slarray, $i_slider, $range) {
			...
			if (JString::substr($search, 0, 1) == '"' && JString::substr($search, $length - 1, 1) == '"') {
				...
				$type = 'any';
                                ...
                                if($slider == 0 and $array == 0) {
									...
									$sql = "AND (i.extra_fields REGEXP '^.*\"$i_slider\",\"value(.[^}]*\"";
									...
									$sql .= ")\".*$')";
										...
										if($slindex) {
											foreach($slindex as $k=>$index) {
												if($k == 0) { 
													$sql = " AND ((i.extra_fields REGEXP '^.*\"$i_slider\",\"value.[^}]*\"$index\".*$')";
												}
		...
		function searchstat() {
			foreach($_GET as $name=>$value) {
				$value = preg_replace("/[^\w_ ]+/u", "", $value);
				switch($name) {
					case "category" :
						if(is_array($value)) {
							$value = $value[0];
						}
						$search_term = "Category: ".$this->getCategoryName($value);
						$this->storestat($search_term);
					break;
					
					case "ftag" :
						$search_term = "Tag: ".$value;
						$this->storestat($search_term);						
					break;
					...
					case (preg_match('/searchword.*/', $name) ? true : false) :
						...
						$search_term = "Extrafield: ".$this->getExtrafieldName($id[0][0]). " -> " .$value;
						$this->storestat($search_term);	
					break;
				}
			}
